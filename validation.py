...
class Cipher135Form(FlaskForm):
    ...
    key = StringField('key', validators=[InputRequired(), Regexp('^[0-9]\d*$', message="Field must be a positive integer."), length(max=135, message="Field cannot exceed 135 digits.")])
    text = TextAreaField('text', validators=[InputRequired(), length(max=1000, message="Field cannot exceed 1000 characters.")])
...
class Cipher101Form(FlaskForm):
    encrypt = SubmitField('encrypt', validators=[Optional()])
    decrypt = SubmitField('decrypt', validators=[Optional()])
    key = StringField('key', validators=[InputRequired(), Regexp('^[0-9]\d*$', message="Field must be a positive integer."), length(max=101, message="Field cannot exceed 101 digits")])
    number = StringField('number', validators=[InputRequired(), Regexp('^[0-9]\d*$', message="Field must be a positive integer."), length(max=135, message="Field cannot exceed 135 digits.")])

# Datatools.
# Basetool.
class BasetoolForm(FlaskForm):
    insequence = StringField('insequence', validators=[InputRequired(), length(max=99, message="Field cannot exceed 99 characters")])
    outsequence = StringField('outsequence', validators=[InputRequired(), length(max=99, message="Field cannot exceed 99 characters")])
    inbase = StringField('inbase', validators=[InputRequired(), Regexp('^0*(?:[2-9]|[1-9]\d\d*)$', message="Must be two or greater integer."), length(max=2, message="Field cannot exceed integer 99.")])
    outbase = StringField('outbase', validators=[InputRequired(), Regexp('^0*(?:[2-9]|[1-9]\d\d*)$', message="Must be two or greater integer."), length(max=2, message="Field cannot exceed integer 99.")])
    number = StringField('number', validators=[InputRequired()])
    def validate_insequence(form, field):
        if form.inbase.validate(form):
            if len(set(field.data[: int(form.inbase.data)])) != len(
                field.data[: int(form.inbase.data)]):
    ...
    def validate_outsequence(form, field):
        if form.outbase.validate(form):
            if len(set(field.data[: int(form.outbase.data)])) != len(
                field.data[: int(form.outbase.data)]):
                raise ValidationError("Sequence must be unique characters.")
    ...
    def validate_number(form, field):
        if form.inbase.validate(form) and form.insequence.validate(BasetoolForm):
