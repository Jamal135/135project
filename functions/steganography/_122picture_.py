...
def load_image(image_name):
    ...
    try: image = Image.open(image_location)
    except: raise ValueError("Image Read Failed")
...
def binary_conversion(input_string, argument):
    ...
    if argument == "binary":
        ...
        byte_list = int(input_string, 2).to_bytes(len(input_string) // 8, byteorder = "big")
...
def integer_conversion(input_integer, argument):
    ...
    else: return int(input_integer, 2)
...
def length_calculator(width, height, data = None):
    ...
    else: return width_length, height_length
...
def capacity_check(data_length, number_points):
    ...
    if remaining_space < 0: 
        raise ValueError(f"Input data length {data_length} exceeds image capacity length {number_points}.")
    return round((100/number_points) * data_length, 2)

# function: rgb_coordinates
def rgb_coordinates(positions, key):
    ''' Returns: List of Randomly Selected RGB Position Values. '''
    seed(key)
    number_values = len(positions)
    return [randint(0, 2) for _ in range(number_values)]
...
def end_point(positions, total_length, width_length, height_length):
    ...
    width_key = integer_conversion(end_position[0], "binary").zfill(width_length)
    height_key = integer_conversion(end_position[1], "binary").zfill(height_length)
    return width_key, height_key

# Function: message_generator
def message_generator(data, width, height, positions):
    ''' Returns: Correctly Built Binary Data to Attach to Image. '''
    total_length, width_length, height_length = length_calculator(width, height, data)
    pixel_utilisation = capacity_check(total_length, len(positions))
    width_key, height_key = end_point(positions, total_length, width_length, height_length)
    message_data = width_key + height_key + data
    length = len(message_data)
    return message_data, length, pixel_utilisation

# Function: extract_values
def extract_values(image, length, positions, rgb_order):
    ''' Returns: Existing Binary Data to be Replaced. '''
    locations = [image.getpixel((positions[point][1], positions[point][0])) for point in range(length)]
    exact_points = [locations[point][rgb_order[point]] for point in range(length)]
    binary_values = [integer_conversion(exact_points[point], "binary").zfill(8) for point in range(length)]
...
def data_comparison(current_values, new_values, length):
    ...
    matches = sum([1 for point in range(length) if current_values[point] == new_values[point]])
...
def data_rebuild(locations, new_values, rgb_order, length):
    ...
    for point in range(length): input_list[point][rgb_order[point]] = new_values[point]
...
def attach_data(image, length, positions, rgb_order, image_message):
    ...
    exact_points = [locations[point][rgb_order[point]] for point in range(length)]
    binary_values = [integer_conversion(exact_points[point], "binary").zfill(8) for point in range(length)]
    new_binary_values = [binary_values[point][0:7] + image_message[point] for point in range(length)]
    new_values = [integer_conversion(new_binary_values[point], "decimal") for point in range(length)]
    new_data = data_rebuild(locations, new_values, rgb_order, length)
    try: [image.putpixel((positions[point][1], positions[point][0]), new_data[point]) for point in range(length)]
    except: raise ValueError("Image Modification Failed.")
    return image
...
def API_image_append(image_name, indata, key:int = 999):
    ...
    rgb_order = rgb_coordinates(positions, key)
    ...
    image_message, length, usage = message_generator(binary_indata, width, height, positions)
    ...
    result_image = attach_data(image, length, positions, rgb_order, image_message)
...
def API_image_extract(image_name, key:int = 999):
    ...
    end_position, key_length = extract_key(image, positions, rgb_order, width_length, height_length)
    # Extract remaining image data from image.
    binary_data = data_extract(image, positions, rgb_order, end_position, key_length)
    # Convert extracted data from binary to plaintext.
    return binary_conversion(binary_data, "decimal")

#print(API_image_append("gate.png", x, 8))
#print(API_image_extract("new_gate.png", 8))
